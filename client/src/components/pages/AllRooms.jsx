    ...
    <div className='flex flex-col-reverse lg:flex-row items-start justify-between pt-28 md:pt-35 px-4 md:px-16 lg:px-24 xl:px-32'>
        <div>
            <div className='flex flex-col item-start text-left'>
                ...
                <p className='text-sm md:text-base text-gray-500/90 mt-2 max-w-170 '>Take advantage of our exclusive selection of premium rooms and suites, each designed to provide comfort and luxury.</p>
            </div>
            {roomsDummyData.map((room) => (
            <div key={room._id} className='flex flex-col md:flex-row items-start py-10 gap-6 border-b border-gray-300 last:pb-30 last:border-0'>
                <img
                onClick={() => {navigate(`/room/${room._id}`); scrollTo(0, 0)}} src={room.images[0]} alt="room-img" title="View Room Details" 
                ...
                <div className='md:w-1/2 flex flex-col gap-2'>
                    ...
                    <div className='flex items-center justify-between'>
                        <p className='text-gray-700 font-medium text-xl'>${room.pricePerNight}/night</p>
                        <button onClick={() => {navigate(`/room/${room._id}`)}} className='px-4 py-2 bg-black text-white rounded-lg cursor-pointer '>
                            Book Now
                        </button>
                    </div>
                </div>
